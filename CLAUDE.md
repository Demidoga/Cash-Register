...
### Database
SQLAlchemy 2.0 + Alembic. Models are DB-agnostic: **tests use in-memory SQLite, production stores data in Supabase Postgres** (set `DATABASE_URL` to Supabase's session-mode pooler string). `db.py` defines `Base`, a rebindable engine/`SessionLocal` (`configure()` for tests), and an explicit constraint `NAMING_CONVENTION` (required for SQLite batch migrations and clean Alembic ALTERs). `make_engine` normalizes a bare `postgresql://` URL to the `psycopg` driver and enables `pool_pre_ping` for Postgres (survives pooler/idle drops); batch migrations are gated to SQLite only. The Alembic URL is injected from app settings in `migrations/env.py`, not hard-coded in `alembic.ini`. Tests build the schema with `Base.metadata.create_all`, not migrations — so when you add a model, you must **both** keep it test-importable and generate an Alembic migration for real deployments. **Supabase here is data + auth; the app reaches it only through the backend, so Row Level Security is not relied on for tenant isolation — `clinic_id` scoping is.**
...
Backend env (see `api/app/config.py`): `DATABASE_URL` (SQLAlchemy URL; defaults to local SQLite), `JWT_SECRET`, `JWT_AUDIENCE` (default `authenticated`), `SUPABASE_URL` (enables JWKS verification), `CLINIC_NAME` (fallback name for first-time setup; the setup wizard sends name + currency), `DEV_LOGIN_ENABLED` (default true — **set false in prod**).
...
**Production checklist:** point `DATABASE_URL` at Supabase's session-mode pooler (transaction-mode pooler ⇒ also set `DB_DISABLE_PREPARED_STATEMENTS=true`), set a strong `JWT_SECRET`, set `DEV_LOGIN_ENABLED=false`, configure the Supabase env vars on both halves. Deploy with `docker compose` (the `db` service was removed — Supabase is the database).
